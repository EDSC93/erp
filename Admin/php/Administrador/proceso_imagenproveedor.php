	...
	<!-- -->
		...
		<h4>Modificar Imagen<h4>
			...
			<?php 
						...
						$result=mysql_query("UPDATE proveedores SET Url_Imagen='$nombre' WHERE ID_Proveedores=$re")or die(mysql_error());
						
						
			
			if($result)
			{
				header("location: modificar_imagenproveedor.php?id=$re");
			}
			else
			{
				echo "No se modificó";
